...
#include <traffic_sim_project_group_2/LaneDetectionConfig.h>
...
class LaneDetection
{
  ...
  private:
    ...
    void Vision_LaneCentering(const std::vector<CurveFit>& curves); 
    void find_Lanes(std::vector<std::vector<geometry_msgs::Point>>&pointcurves);

    std::vector<std::vector<geometry_msgs::Point>> All_pointcurves;
    std::vector<geometry_msgs::Point> centerline_points_to_follow; 
