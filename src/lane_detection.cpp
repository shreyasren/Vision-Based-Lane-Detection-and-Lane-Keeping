...
namespace traffic_sim_project_group_2
{
  ...
  kd_tree_(new pcl::search::KdTree<pcl::PointXYZ>)
{
  ...
  pub_LaneCenter_ = n.advertise<visualization_msgs::MarkerArray>("Lane_Center", 1);
  pub_ulc_cmd_ = n.advertise<dataspeed_ulc_msgs::UlcCmd>("trajectory_ulc_cmd", 1);
...
/******************************************************************************************
Function : Vision_LaneCentering
Description: Find Lane Centre Curve and publish the Markers and point to ULC cmd
Parameters: vector<CurveFit>& curves
Return: None
/******************************************************************************************/
void LaneDetection::Vision_LaneCentering(const std::vector<CurveFit>& curves)
{

  /*******  Publish Trajectory_ulc_cmd *******************/
  // Transform each waypoint into vehicle frame coordinates
  std::vector<avs_lecture_msgs::PointWithSpeed> vehicle_frame_points;
  ...
  All_pointcurves.resize(marker_msg.markers.size());
  for(size_t i=0; i< marker_msg.markers.size();i++){
    All_pointcurves[i]=marker_msg.markers[i].points;
  }
  
  if(All_pointcurves.size() >= 2) // We must have 2 lanes in minimum
  {
    // Find the Center Lane from the all the point curves
    find_Lanes(All_pointcurves);
  }


  // Select a signal point at particular distance in fornt of the vehicle
  
  avs_lecture_msgs::PointWithSpeed target_point;
  
  // 6.5 is having Lat Erro of 0.145 
  double Lookahead_dist = 6.5; 
  if(cfg_.Enb_Dis_CfgLookAhead == true)
  {
    Lookahead_dist = cfg_.lookahead_dist;
  }

  //Clear all Lane marks to record only Lane center path to mark on Rviz  
  viz_marker.id = 0;
  viz_marker.points.clear();
  marker_msg.markers.clear();

  for (int i = 0 ; i < centerline_points_to_follow.size(); i++)
  {
    
    if (centerline_points_to_follow[i].x >= Lookahead_dist)
    {
      target_point.point.x = centerline_points_to_follow[i].x;
      target_point.point.y = centerline_points_to_follow[i].y; 
      break; 
    }
    
  }
      // include the centerline_points_to_follow
      viz_marker.points = centerline_points_to_follow;
      marker_msg.markers.push_back(viz_marker);
      viz_marker.id++;

  /*******  Publish Trajectory_ulc_cmd *******************/
  // Pass the traget coordinates into the curvature equation
  // Equ: k= (2*yt)/(x^2+y^2)
  double k_result; //result of curvature equation
  k_result= (2*target_point.point.y)/((pow(target_point.point.x,2))+(pow(target_point.point.y,2)));
  // Fill in ULC command message with the curvature
  ...
  ulc_cmd_msg.yaw_command = k_result;//cfg_.yaw_command;
  ulc_cmd_msg.steering_mode = 1;//cfg_.steering_mode;

  // Set the speed of the ULC command message to match the speed of the selected point
  ulc_cmd_msg.linear_velocity = cfg_.linear_velocity;
  // Publish ULC command
  // Publish message
  pub_ulc_cmd_.publish(ulc_cmd_msg);

  //END *******  Publish Trajectory_ulc_cmd *******************/
  ...
  delete_markers.markers.resize(1);
  delete_markers.markers[0].action = visualization_msgs::Marker::DELETEALL;
  pub_LaneCenter_.publish(delete_markers);

  //Publish for visualization
  pub_LaneCenter_.publish(marker_msg);


}

void LaneDetection::find_Lanes(std::vector<std::vector<geometry_msgs::Point>>&pointcurves)
{
  
  std::vector<std::vector<geometry_msgs::Point>> right_pointcurves; //it is a vector of lines
  std::vector<std::vector<geometry_msgs::Point>> left_pointcurves; //it is a vector of lines

  // looping through all the lines i represents lines index
  for (size_t i=0;  i<pointcurves.size(); i++) 
  {
    int small, large;
    // if the Y coordinate of the first point of line i is minus ( minus means it's to the left of the car)
    if(pointcurves[i][0].y < 0 ) 
    {
      // add this line to the negative vector 
      // negative values are sorted in negative-lines
       left_pointcurves.push_back(pointcurves[i]);   
      
    }
    // if the Y coordinate of the first point of line i is positive 
    else if (pointcurves[i][0].y > 0 )
    {
      right_pointcurves.push_back(pointcurves[i]);
    }
  }
  
  // go throuh all the left  lines and find the greater one in terms of Y values of thier first point
  if (right_pointcurves.size() <1 && left_pointcurves.size() <1)
  return;
  int largest_distance = INT_MIN; //  largest
  int left_index ; //  is less than every number you can write 
  for (size_t g =0; g < left_pointcurves.size(); g++)
  {
    if (left_pointcurves[g][0].y > largest_distance )
    {
      largest_distance = left_pointcurves[g][0].y;
      left_index = g;
    }
  }

  int right_index ;// this index represents the closer line to the car
  int smallest_distance = INT_MAX;      
 // get the smallest size i.e. index of immediate lane lines each on right
  for (size_t g =0; g < right_pointcurves.size(); g++) 
  {
    if (right_pointcurves[g][0].y < smallest_distance )
    {
      smallest_distance = right_pointcurves[g][0].y;
      right_index = g;
    }
  }
  ...
  // get the smallest size i.e. index of immediate lane lines each on left 
  if (left_pointcurves[left_index].size() < right_pointcurves[right_index].size())
  { 
    center_size = left_pointcurves[left_index].size(); 
    centerline_points_to_follow.resize(center_size);
  }
  else 
  { 
    center_size = right_pointcurves[left_index].size();  
    centerline_points_to_follow.resize(center_size);
  }
  // Finding average of the points on two immediate lane lines each on left and right and save it in a new line
  for (size_t i=0; i<center_size; i++)
  {
    centerline_points_to_follow.at(i).y = (left_pointcurves[left_index].at(i).y + right_pointcurves[right_index].at(i).y )/2;
    centerline_points_to_follow.at(i).x = (left_pointcurves[left_index].at(i).x + right_pointcurves[right_index].at(i).x )/2;
    centerline_points_to_follow.at(i).z = 0;
  }
